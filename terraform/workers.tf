resource "openstack_networking_floatingip_v2" "kuber-worker" {
...
resource "template_file" "kuber-address-lines" {
    ...
    vars {
        ...
        ip_address = "${element(openstack_networking_floatingip_v2.kuber-worker.*.address, count.index)}"
    }
}
...
resource "template_file" "worker-user-data" {
    ...
    vars {
        ...
        public_ipv4 = "${element(openstack_networking_floatingip_v2.kuber-worker.*.address, count.index)}"
...
resource "openstack_compute_instance_v2" "worker" {
    ...
    floating_ip = "${element(openstack_networking_floatingip_v2.kuber-worker.*.address, count.index)}"
